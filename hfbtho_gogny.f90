...
module hfbtho_gogny
  ...
  real(pr), Private, allocatable, dimension(:,:,:) :: T_z  !< \f$ T_{n_1 n_2}^n \f$= Tz(n1,n2,n) coefficients of the axial Gogny expansion
  ...
  real(pr), Private, allocatable, dimension(:,:,:) :: Cp2c !< \f$ C^{n_r k}_{n_y}\f$= Cp2c(nr,k,ny) coefficients of the radial to cartesian
...
contains
  ...
  subroutine CalculateVzGogny()
    ...
    allocate(VzGogny(1:n_g,0:ii))
    VzGogny(1:n_g,0) = zero
!$OMP Parallel Default(None) &
!$OMP& SHARED(n,Zblock,VzGogny,mu_g,bz,n_g) &
       ...
       do nzj = 0,n
           do nzk = 0,n
             do nzl = 0,n
                ...
                do ig = 1,n_g
                   Vz = MatrixElement_z(nzi,nzj,nzk,nzl,mu_g(ig),bz)
                   VzGogny(ig,ii) = Vz
                enddo
             enddo
          enddo
...
!$OMP End Parallel
    ...
    allocate(Vz_Gogny(1:n_g,0:n,0:n,0:n,0:n))
    Vz_gogny = zero
    do nzi = 0,n
       do nzj = 0,n
           do nzk = 0,n
...
!$OMP Parallel Default(None) &
!$OMP& SHARED(ilauf,nr_flag,nl_flag,ir_flag,VrGogny,n_g) &
...
!$OMP End Parallel
    ...
    allocate(Vr_Gogny(1:n_g,0:n/2,-n:n,0:n/2,-n:n,0:n/2,-n:n,0:n/2,-n:n))
    Vr_gogny = zero
    do nra = 0,n/2
       do nrb = 0,n/2
          do nrc = 0,n/2
...
!$OMP End Parallel
  ...
  subroutine calculateME1D()
    ...
    allocate(ME1D(1:N_g,0:n,0:n,0:n,0:n))
    ME1D = zero
!$OMP Parallel Default(None) &
!$OMP& SHARED(n,ME1D,mu_g,bp,n_g) &
       ...
       do nj = 0,n
          do nk = 0,n
             do nl = 0,n
                ...
                do ig = 1,n_g
                   ME = MatrixElement_z(ni,nj,nk,nl,mu_g(ig),bp)
...
!$OMP End Parallel
  ...
  subroutine radial_matrix_elements(ni,li,nj,lj,nk,lk,nl,ll)
    ...
    if(.not.allocated(Vr_ig)) allocate(Vr_ig(1:n_g))
    Vr_ig = zero
...
#else
    do ig = 1,n_g
       Vr_ig(ig) = MatrixElement_r(ni,li,nj,lj,nk,lk,nl,ll,mu_g(ig),bp)
    enddo
#endif
