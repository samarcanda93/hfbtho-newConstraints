...
Module hfbtho_fission_fragments
  ...
  Real(pr), PUBLIC, SAVE :: DFRAG=0.0_pr
  Real(pr), PUBLIC, SAVE :: CSI=0.0_pr
  ! Fission fragments
...
Contains
  ...
  Subroutine DEFMAS(NZMAXX,ZPOINT,SFACTO)
    ...
    Allocate(PHERMI(0:NORDER)); Allocate(DHERMI(0:NORDER))
    Call D_HERM(XPOINT,NORDER,PHERMI,DHERMI,NORDER)
    !
    ! Compute value of Hermite polynomials up to order Max(Nz+Mz) at z=Abs(ZPOINT)
    XPOINT=Abs(ZPOINT); NORDER=2*NZMAXX
    Allocate(ZHERMI(0:NORDER))
    Call D_HERM(XPOINT,NORDER,ZHERMI,DHERMI,NORDER)
    !
    ! Loop over bra and ket z quantum number
    Do NZ=0,NZMAXX
       Do MZ=0,NZMAXX
          ! Initial value of the sum for K = 0. Coefficients COEF00 are scaled by
          ! SQRT(bz) in DEVHER, we need to remove this scaling here
          SUMVAL=COEF00(0,MZ,NZ)*Sqrt(bz) * 0.5_pr*Sqrt(Sqrt(PIARGU))*(1.0_pr - VALINT)
          If(ZPOINT.Lt.0.0_pr) SUMVAL=COEF00(0,MZ,NZ)*Sqrt(bz)* 0.5_pr*Sqrt(Sqrt(PIARGU))*(1.0_pr + VALINT)
          Do K=1,NZ+MZ
             ...
             Else
                F_INTE =(ZHERMI(K)*Exp(-ZPOINT*ZPOINT) - PHERMI(K-1)) / HERFAC(K)
                If(ZPOINT.Lt.0.0_pr) F_INTE = - F_INTE
             End If
             SUMVAL=SUMVAL+COEF00(K,MZ,NZ)*Sqrt(bz) * F_INTE
          End Do
          SFACTO(MZ,NZ)=SUMVAL
       End Do
    End Do
  ...
  Subroutine print_moments(iw)
    ...
    Write(iw,'(2X,"Fragment distance ...",2f12.4)') DFRAG
    Write(iw,'(2X,"Mass asymmetry ......",2f12.4)') CSI
  ...
  !=======================================================================
    ...
    !====================================================================
  !> The routine calculates the distance between the fragments
  !====================================================================
  ...
  !====================================================================
  !> The routine calculates the mass difference between the fragments
  !====================================================================
  ...
  End Subroutine CSICALC
    ...
    !=======================================================================
  !> Calculates the matrix of the fragment distance operator in the current
  !> block. The result is stored in the global variable gaussian_neck(:)
  !=======================================================================
  ...
  Subroutine csi_computeField(ib)
    ...
    !
    Return
  End Subroutine csi_computeField
