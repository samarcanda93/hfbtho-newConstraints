...
Contains
  ...
  Subroutine HFBTHO_DFT_SOLVER
    ...
    Do
       ...
       End If
       !
       ! Add constraint on the mass asymmetry
...
!$OMP END PARALLEL
  ...
  Subroutine thoalloc
    ...
    Allocate(qfield(nghl,lambdaMax+3)) ! constraining fields: lambdaMax multipoles + neck + ff distance + mass asymmetry
    ...
    If(lpr) Then
       Do iw=lout,lfile
          ...
          Else
             ...
             If(dfrag_constraints) Then
                icount=icount+1
                Write(iw,'(a,i1,a,a,f8.3)') '    Dfrag       ',icount,' .............: lambda=0', &
                     ' Ql=',dfragRequested
             End If
             If(csi_constraints) Then
                icount=icount+1
                Write(iw,'(a,i1,a,a,f8.3)') '    Csi       ',icount,' .............: lambda=0', &
                     ' Ql=',csiRequested
             End If
...
!$OMP End Parallel
  ...
  Subroutine resu(irecord)
    ...
    If(write_hel.and.npr(1).Eq.2*(npr(1)/2).And.npr(2).Eq.2*(npr(2)/2)) Then
       Call inout(2,iexit)
       If(ierror_flag.Ne.0) Return
    End If
    !--------------------------------------------
...
101                    Format(i4,2f12.6,2(1x,f12.8),2(2x,f7.4),' ',a13)
  ...
  Subroutine initialize_HFBTHO_NAMELIST
    ...
    neck_value             = 0.5_pr
    ! HFBTHO_DFRAG
    set_dfrag_constrain     = .False.
    dfrag_value             = 0.5_pr
    ! HFBTHO_CSI
    set_csi_constrain     = .False.
    csi_value             = 0.5_pr
    ! HFBTHO_DEBUG
    number_Gauss        =  40
    number_Laguerre     =  40
  ...
  Subroutine read_HFBTHO_NAMELIST
    ...
    End If
    !
        ! Constraint on the fragment distance
    Read(UNIT=lnamelist,NML=HFBTHO_DFRAG,iostat=ios)
    If (ios.Ne.0) Then
       ierror_flag=ierror_flag+1
       ierror_info(ierror_flag)='Error in HFBTHO_DFRAG read'
       Return
    End If
    !
        ! Constraint on the mass asymmetry
    Read(UNIT=lnamelist,NML=HFBTHO_CSI,iostat=ios)
    If (ios.Ne.0) Then
       ierror_flag=ierror_flag+1
       ierror_info(ierror_flag)='Error in HFBTHO_CSI read'
       Return
    End If
    !
    ! Debug
...
100                      Format(i2,a1,'[',i2,',',i2,',',i2,']')
   ...
   nb=ib;  nt=ilauf
    ...
    If(neck_constraints.or.dfrag_constraints.or.csi_constraints) Then
       Allocate(brin(nhhdim4+lambdaMax+3),brout(nhhdim4+lambdaMax+3))
    Else
...
!$OMP End Parallel Do
  ...
  Subroutine field
    Implicit None
    Integer(ipr) :: iw,it,ita,ihli,lambda,icons,ic
...
!                pUr(it)=pUr(it)/(QLMRIG+QLMLEF)
  ...
  Subroutine constraining_field()
    ...
    Do ihli=1,nghl
       qfield(ihli,lambdaMax+1)=zero
       qfield(ihli,lambdaMax+2)=zero
       qfield(ihli,lambdaMax+3)=zero

       
       z=fh(ihli)

       If (neck_constraints.and.ic.gt.0) Then
          qfield(ihli,lambdaMax+1) = Exp(-((z-Z_NECK*bz)/AN_VAL)**2)
          ic = ic -1
       End If
...
!$OMP End Parallel Do
    ...
    !
    if(.not.WoodsSaxon.and.finite_range) then
    ...
    End If
    If(dfrag_constraints) Then
       brout(nhhdim4+lambdaMax+2)=dfragLag
    End If
    If(csi_constraints) Then
       brout(nhhdim4+lambdaMax+3)=csiLag
    End If
    !----------------------------------------------
    ...
    If(DoMixing) Then
       ...
       If(neck_constraints.or.dfrag_constraints.or.csi_constraints) Then
          Call broyden_min(nhhdim4+lambdaMax+3,brout,brin,alphamix,si,iiter,nbroyden,bbroyden)
       Else
    ...
    Else
       brin(nhhdim4+lambdaMax+1)=zero
    End If
    If(dfrag_constraints) Then
       dfragLag=brin(nhhdim4+lambdaMax+2)
    Else
       brin(nhhdim4+lambdaMax+2)=zero
    End If
    If(csi_constraints) Then
       csiLag=brin(nhhdim4+lambdaMax+3)
    Else
       brin(nhhdim4+lambdaMax+3)=zero
    End If
    !
  ...
  Subroutine gamdel_gogny
    ...
    real(pr) :: Zrho_n_dir(1:n_g,0:nrx,0:nrx,0:1,0:1,1:nox+1)
    real(pr) :: Zrho_p_dir(1:n_g,0:nrx,0:nrx,0:1,0:1,1:nox+1)
    real(pr) :: Zrho_n_exc(1:n_g,0:nrx,0:nrx,0:1,0:1,1:nox+1)
    real(pr) :: Zrho_p_exc(1:n_g,0:nrx,0:nrx,0:1,0:1,1:nox+1)
    real(pr) :: Zkap_n_dir(1:n_g,0:nrx,0:nrx,0:1,0:1,1:nox+1)
    real(pr) :: Zkap_p_dir(1:n_g,0:nrx,0:nrx,0:1,0:1,1:nox+1)
    ...
    if(allocated(gamma_g_dir)) deallocate(gamma_g_dir,gamma_g_exc,delta_g_dir)
    allocate(gamma_g_dir(ndx**2,2*nbx),gamma_g_exc(ndx**2,2*nbx),delta_g_dir(ndx**2,2*nbx))
    gamma_g_dir = zero
    gamma_g_exc = zero
    delta_g_dir = zero
...
!$OMP&        gamma_g_dir,gamma_g_exc,delta_g_dir,brout,n_g) &
...
!$OMP End Parallel
  ...
  Subroutine expectpj(lpr)
    ...
    Implicit None
    Logical :: lpr
    Integer(ipr) :: i,j,it,ihli,iw,iw1=901,iw2=902,icons,lambda,ign,igp,k,ic
    ...
    If(icstr.Ne.0) Then
       ...
       If (numberCons.Gt.0) Then
           ...
           If (neck_constraints) cx = cx - neckLag*(neckValue-neckRequested)
           If (dfrag_constraints) cx = cx - dfragLag*(dfragValue-dfragRequested)
           If (csi_constraints) cx = cx - csiLag*(csiValue-csiRequested)
...
100        Continue                        ! complications are due to eagle_ornl
  ...
  Subroutine expect(lpr)
    ...
    Real(pr) :: QLMLEF,QLMRIG
    Real(pr) :: RpRMSsq,RnRMSsq,DarwinFoldy
    Real(pr) :: e_gamma_fr_dir,e_gamma_fr_exc,delrho1,delrho2,delkap1,delkap2
    Real(pr), Pointer     :: EqpPo(:),VqpPo(:),UqpPo(:)
    Integer(ipr), Pointer :: KpwiPo(:),KqpPo(:)
    ...
    etot=ekt(3)+evol+esurf+eso+ecoul+ept(3)+ETENS+EExtra+E_EXT_FIELD&
         + e_gamma_fr_dir+e_gamma_fr_exc + frept(3)!e_delta_fr
    ehfb=etot
    ...
    If (lpr) Then
       ...
       Do iw=lout,lfile
          ...
          Write(iw,'(a,3f15.6)') '  delta(n,p), pwi .....',del+frdel,pwi
          If(finite_range) then
          ...
          If(kindhfb.Lt.0) Then
             ...
             Write(iw,'(a,3f15.6)') '  delta(n,p)+ala2 .....',del+frdel+ala2
  ...
  Subroutine getLagrange(ite)
    ...
    Integer(ipr) :: i_uvN,i_uvP,incx,incy,ic
    ...
    If (neck_constraints.and.ic.gt.0) Then
       ...
       If (nbroyden.lt.1) Then
          vecold(numberCons-ic+1)=neckLag
       Else
          vecold(numberCons-ic+1)=brin(nhhdim4+lambdaMax+1)
       End If
    ...
    If (csi_constraints.and.ic.gt.0) Then
       ...
       If (nbroyden.lt.1) Then
          vecold(numberCons-ic+1)=csiLag
       Else
          vecold(numberCons-ic+1)=brin(nhhdim4+lambdaMax+3)
       End If
       veclam(numberCons-ic+1)=vecold(numberCons-ic+1)
       ic = ic - 1
    End If
    ...
    Do ib = 1,nb
       ...
       If(kd(ib,it).Gt.0) Then
          ...
          If (neck_constraints.and.ic.gt.0) Then
             ...
             ! second term: v^{+} f^{*} u^{*} = v^{T} f u
             Call dgemm('n','n',nd,kd(ib,it),nd,one,dblmul,nd,Umatr,nd,zero,doubln,nd)
             Call dgemm('t','n',kd(ib,it),kd(ib,it),nd,one,Vmatr,nd,doubln,nd,zero,fn12pl(1,1,numberCons-ic+1),kd(ib,it))
             !
             ! first term:  u^{+} f v^{*} = u^{T} f v
             Call dgemm('n','n',nd,kd(ib,it),nd,one,dblmul,nd,Vmatr,nd,zero,doubln,nd)
             Call dgemm('t','n',kd(ib,it),kd(ib,it),nd,one,Umatr,nd,doubln,nd,minu,fn12pl(1,1,numberCons-ic+1),kd(ib,it))
             !
             ! temperature - computing \tilde{f}^{11}
             If(switch_on_temperature) Then
                !
          ...
          If (dfrag_constraints.and.ic.gt.0) Then
            ...
            ic =ic -1
             !
            If(Allocated(ddfrag)) Deallocate(ddfrag)                        
         ...
         If (csi_constraints.and.ic.gt.0) Then
            ...
            End Do
             ...
             !
             If(Allocated(ccsi)) Deallocate(ccsi)
             Deallocate(dblmul)
             !
      ...
      End If
       ...
       If(kd(ib,it).Gt.0) Then
          ...
          If (neck_constraints.and.ic.gt.0) Then
             ...
             ! second term: v^{+} f^{*} u^{*} = v^{t} f u
             Call dgemm('n','n',nd,kd(ib,it),nd,one,dblmul,nd,Umatr,nd,zero,doublp,nd)
             Call dgemm('t','n',kd(ib,it),kd(ib,it),nd,one,Vmatr,nd,doublp,nd,zero,fp12pl(1,1,numberCons-ic+1),kd(ib,it))
             !
             ! first term:  u^{+} f v^{*} = u^{t} f v
             Call dgemm('n','n',nd,kd(ib,it),nd,one,dblmul,nd,Vmatr,nd,zero,doublp,nd)
             Call dgemm('t','n',kd(ib,it),kd(ib,it),nd,one,Umatr,nd,doublp,nd,minu,fp12pl(1,1,numberCons-ic+1),kd(ib,it))
             !
             ! temperature - computing \tilde{f}^{11}
             If(switch_on_temperature) Then
               !
               ! second term: v f^{*} v = v^{T} f v
               Call dgemm('n','n',nd,kd(ib,it),nd,one,dblmul,nd,Vmatr,nd,zero,doublp,nd)
               Call dgemm('t','n',kd(ib,it),kd(ib,it),nd,one,Vmatr,nd,doublp,nd,zero,fp11pl(1,1,numberCons-ic+1),kd(ib,it))
               ! first term:  u f u = u^{T} f u
               Call dgemm('n','n',nd,kd(ib,it),nd,one,dblmul,nd,Umatr,nd,zero,doublp,nd)
               Call dgemm('t','n',kd(ib,it),kd(ib,it),nd,one,Umatr,nd,doublp,nd,minu,fp11pl(1,1,numberCons-ic+1),kd(ib,it))
               !
             End If
             !
             ic = ic - 1
    ...
    If (neck_constraints.and.ic.gt.0) Then
       veccns(numberCons-ic+1)=veclam(numberCons-ic+1)+workcn(numberCons-ic+1)
       If (nbroyden.lt.1) Then
          neckLag=brakev*vecold(numberCons-ic+1)+(1.0-brakev)*veccns(numberCons-ic+1)
       Else
          neckLag=veccns(numberCons-ic+1)
          brout(nhhdim4+lambdaMax+1)=neckLag
       End If
       ic = ic - 1
    End If
    ...
    If (dfrag_constraints.and.ic.gt.0) Then
       veccns(numberCons-ic+1)=veclam(numberCons-ic+1)+workcn(numberCons-ic+1)
       If (nbroyden.lt.1) Then
          dfragLag=brakev*vecold(numberCons-ic+1)+(1.0-brakev)*veccns(numberCons-ic+1)
       Else
          dfragLag=veccns(numberCons-ic+1)
          brout(nhhdim4+lambdaMax+2)=dfragLag
       End If
       ic = ic - 1
    End If
    ...
    If (csi_constraints.and.ic.gt.0) Then
       veccns(numberCons-ic+1)=veclam(numberCons-ic+1)+workcn(numberCons-ic+1)
       If (nbroyden.lt.1) Then
          csiLag=brakev*vecold(numberCons-ic+1)+(1.0-brakev)*veccns(numberCons-ic+1)
       Else
          csiLag=veccns(numberCons-ic+1)
          brout(nhhdim4+lambdaMax+3)=csiLag
       End If
       ic = ic - 1
    End If
